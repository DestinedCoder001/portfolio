  ...
  return (
    ...
    >
      <div className="flex justify-between items-center h-[60px]">
        ...
        <div onClick={() => setOpen(!open)} className="block md:hidden active:bg-white/20 rounded-full">
      ...
      >
        {
          navLinks.map((link) => (
            <NavBarLink key={link.href} href={link.href} text={link.text} onClick={() => setOpen(false)} />
          ))
        }
        <Link href="https://github.com/Olowokere-Destiny" target="_blank" onClick={() => setOpen(false)}>
          <Image
            src={github}
            width={100}
            height={100}
